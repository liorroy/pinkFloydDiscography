        ...
        with open(file_path, 'r') as file:
            ...
            for line in file:
                ...
                if line.startswith('#'):  # new album
                    ...
                    print(album_name, "= CURRENT ALBUM NAME = ", current_album_dict.get("album_name"), "=", discography_dict[album_name]["album_name"], "=", discography_dict[current_album_dict.get("album_name")]["album_name"])   # print(discography_dict)
                    ...
                    print("\t\t\t\t\t\t", "current song_name = ", song_name, " len = ", len(song_name))
                    print("\t\t\t\t\t\t", "current song_writer = ", song_writer, " len = ", len(song_writer))
                    ...
                    # add the new song dict to the songs list for the current album (a list of dicts - of songs)
                    # in discography_dict, in the current album's value (a dict),
                    discography_dict[current_album_dict.get("album_name")]["songs_list"].append(current_song_dict)
                    # discography_dict[current_album_dict.get(album_name)]['songs_list'].append(current_song_dict)
                    # discography_dict[current_album_dict]['songs'][song_name] = song_name
                    # discography_dict[current_album_dict]['songs'][song_writer] = song_writer
                    # discography_dict[current_album_dict]['songs'][song_length] = song_length
                    # discography_dict[current_album_dict]['songs'][song_lyrics] = song_lyrics
                elif current_album_dict and current_song_dict:
                    # print("\t\t\tNot a new song / album:\t\r", line, "\ncurrent song lyrics in dict: ",
                    #       discography_dict[current_album_dict.get("album_name")]["songs_list"][-1]['song_lyrics'])
                    print("\t\t\tNot a new song / album:\t\r", line, "\ncurrent song lyrics in dict: ",
                          discography_dict[current_album_dict.get("album_name")]["songs_list"][-1]['song_lyrics'])
                    # discography_dict[current_album_dict.get("album_name")]["songs_list"].append(current_song_dict)
                    # discography_dict[current_album_dict.get("album_name")]["songs_list"].append(current_song_dict)
                    # print(album_details)
                    # print(current_album_dict)
                    print("\t\t\tNot a new song / album:\t\r", line, "\ncurrent song lyrics in dict: ", discography_dict[current_album_dict.get("album_name")]["songs_list"][-1]['song_lyrics'])
                    # discography_dict[current_album_dict.get("album_name")]["songs_list"].append(current_song_dict)
                    lyrics_up_to_this_line_in__dict = discography_dict[current_album_dict.get("album_name")]["songs_list"][-1]['song_lyrics']
                    discography_dict[current_album_dict.get("album_name")]["songs_list"][-1]['song_lyrics'] = lyrics_up_to_this_line_in__dict + " " + line
    except OSError as e:
        print(e)
        return None
    print("\n\n\n\n\n\tThe returned dict:"'\n', discography_dict.keys(), '\n\n\n', discography_dict.items(), '\n\n\n',
          discography_dict, '\n\n\n', discography_dict.get("The Piper At The Gates Of Dawn"),
          '\n\n\n', discography_dict.get("The Piper At The Gates Of Dawn!", "MISSING KEY!"),
          '\n\n\n', discography_dict["The Piper At The Gates Of Dawn"])
