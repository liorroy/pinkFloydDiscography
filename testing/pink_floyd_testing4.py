    ...
    try:
        with open(file_path, 'r') as file:
            ...
            for line in file:
                ...
                # # Check if the line is an album header
                # if line.startswith('#'):
                #     album_details = line[1:].split('::')
                #     album_title = album_details[0].strip()
                #     album_year = album_details[1].strip()
                #
                #     # Initialize a new album entry
                #     current_album = {
                #         'title': album_title,
                #         'year': album_year,
                #         'songs': []
                #     }
                #     discography[album_title] = current_album

                # if line.startswith('#'):
                #     album_details = line[1:].split('::')
                #     current_album = {
                #         'title': album_details[0].strip(),
                #         'year': album_details[1].strip(),
                #         'songs': []
                #     }
                #     discography[current_album['title']] = current_album
